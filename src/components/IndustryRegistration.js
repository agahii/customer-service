...
const IndustryRegistration = () => {
    ...
    const handleSubmit = (values) => {
        ...
        if (isEditing && selectedRecord) {
            ...
            dispatch(updateIndustryRegistration(updatedPayload))
                ...
                .catch((err) => {
                    message.error(`Update failed: ${err.reasonPhrase || err}`);
                });
        } else {
            dispatch(addIndustryRegistration(payload))
                ...
                .catch((err) => {
                    message.error(`Addition failed: ${err.reasonPhrase || err}`);
                });
        }
    ...
    useEffect(() => {
        ...
        dispatch(fetchIndustry({ pagingInfo, controller }))
            .unwrap()
            .catch((err) => {
                message.error(`Fetching industries failed: ${err.reasonPhrase || err}`);
            });
    ...
    useEffect(() => {
        if (error) {
            message.error(`Error: ${error.reasonPhrase || error}`);
        }
    }, [error]);
        ...
        Modal.confirm({
            ...
            onOk: () => {
                dispatch(deleteIndustryRegistration(record.id))
                    ...
                    .catch((err) => {
                        message.error(`Deletion failed: ${err.reasonPhrase || err}`);
                    });
            },
        });
    };
